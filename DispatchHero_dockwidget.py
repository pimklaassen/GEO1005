...
#################################################################################################################
    ...
    def autoOff(self):
        ...
        #function taken over from the MapTool class
...
###################################################################################################
    ...
    def importBridgesData(self, filename=''):
        new_file = None
        try:
            new_file = QtGui.QFileDialog.getOpenFileName(self, "", self.projectPath, '(*.txt)')
            if not new_file:
                return
            path_name = '/'.join(unicode(new_file).split('/')[:-1])
...
class MapTool(QgsMapTool):
    def __init__(self, canvas, iface):
        ...
        #show the firestation on the map
        self.firestation_coord = (92619.8, 436539)
        self.rubberBandFirestation = QgsRubberBand(self.canvas, True)
        ptstoadd = []
    ...
    def clear(self):
        ...
        self.canvas.scene().removeItem(self.rubberBandFirestation)
